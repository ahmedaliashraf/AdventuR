...
public class Sprite extends Rect {
	...
	public Sprite(int x, int y, int w, int h, String filename, int duration, String[] actions, int[] image_counts) {
		super(x, y, w, h);

		animation = new Animation[actions.length];

		for (int i = 0; i < actions.length; i++) {
			animation[i] = new Animation(filename + "_" + actions[i] + "_", image_counts[i], duration);
